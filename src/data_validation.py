...
class DataValidator:
    ...
    def validate_ranges(self, df):
        ...
        for feat, (min_val, max_val) in self.ranges.items():
            if feat in df.columns:
                ...
                out_of_range = real_values[
                    (real_values < min_val) | (real_values > max_val)
                ]

                missing_count = len(df[df[feat] == 0])
                if missing_count > 0:
                    print(
                        f"[INFO] {feat}: {missing_count} samples have missing data (0.0)"
                    )

                if not out_of_range.empty:
                    print(
                        f"[WARNING] {feat}: {len(out_of_range)} genuine outliers detected out of range {min_val}-{max_val}"
                    )
        return True
    ...
    def check_nulls(self, df, threshold=0.3):
        ...
        if not high_nulls.empty:
            print(
                f"[ERROR] Columns with too many nulls (> {threshold*100}%):\n{high_nulls}"
            )
            return False
        return True
...
def main():
    ...
    if not validator.validate_schema(df):
        success = False
    if not validator.validate_ranges(df):
        success = False
    if not validator.check_nulls(df):
        success = False
