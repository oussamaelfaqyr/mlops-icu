...
def compute_risk_features(series, vital_name=None):
    ...
    normals = {"heart_rate": 80, "spo2": 98, "resp_rate": 16, "temp": 37, "mean_bp": 90}

    if series.empty:
        return {"mean": 0, "std": 0, "trend": 0, "dev_from_normal": 0}
...
def generate_sliding_window_features(
    vitals_path, stays_path, obs_hours=12, gap_hours=6, pred_hours=6
):
    ...
    for stay_id, group in df_vitals.groupby("stay_id"):
        ...
        while current_time + timedelta(hours=gap_hours + pred_hours) <= end_time:
            # Observation Window
            obs_window = group[
                (group["charttime"] >= current_time - timedelta(hours=obs_hours))
            ...
            pred_window = group[
                (group["charttime"] >= pred_start) & (group["charttime"] < pred_end)
            ]
            ...
            is_deteriorated = (
                pred_window[
                    (pred_window["vital_name"] == "spo2")
                    & (pred_window["valuenum"] < 90)
                ].shape[0]
                > 0
            ) or (
                pred_window[
                    (pred_window["vital_name"] == "heart_rate")
                    & (pred_window["valuenum"] > 110)
                ].shape[0]
                > 0
            )
    ...
    df_final = df_final.merge(
        df_stays[["stay_id", "subject_id", "gender", "anchor_age"]],
        on="stay_id",
        how="left",
    )
...
if __name__ == "__main__":
    ...
    df_features = generate_sliding_window_features(
        DATA_DIR / "vitals_cleaned.csv", DATA_DIR / "stays_cleaned.csv"
    )
    df_features.to_csv(DATA_DIR / "features_final.csv", index=False)
    print(
        f"Generated {len(df_features)} samples. Target distribution:\n{df_features['target'].value_counts()}"
    )
